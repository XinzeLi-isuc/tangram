...
Each config runs in its own subprocess (_smoke_one.py) to avoid
CUDA context contamination. Accumulates failures and exits non-zero
on any failure so CI can catch regressions.
...
def main():
    if not os.path.isfile(SMOKE_SCRIPT):
        raise SystemExit(f"{SMOKE_SCRIPT} not found (must be committed)")
